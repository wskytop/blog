...
概念：仅在 Web 页面初始化时加载相应的 HTML、JavaScript 和 CSS。一旦页面加载完成，SPA 不会因为用户的操作而进行页面的重新加载或跳转；取而代之的是利用路由机制实现 HTML 内容的变换，UI 与用户的交互，避免页面的重新加载。
...
4. 数据：数据没有使用劫持的模式，而是通过重写数据原型上的方法来实现数组的响应式
...
-   `beforeCreate`: 实例Vue，未初始化和响应式数据
    `created`： 已初始化和响应式数据，可访问数据；在这里可以发送请求
-   `beforeMount`：render调用，虚拟DOM生成，未转真实DOM,可以发起服务端请求，去数据，ssr中不可用，基本用不上这个hook
    `mounted`: 真实DOM挂载完毕,SSR中不可用
...
​										 2. 指令所在的模板被重新解析时调用
   ...
   2. 对象式
      ...
      - bind(): 指令与元素成功绑定时调用
      - inserted(): 指令所在元素被插入页面时被调用,模板被重新解析时不会调用
      - update(): 指令所在的模板被重新解析时调用

​            3. 全局指令 ：Vue.directive(指令名, 配置项：(函数/对象))

> 指令名 ：多个单词采用 - 连接
>
> directives 中的 this 指向 window

### Vue.set()方法和$set()

> 向响应式对象中添加一个property，确保这个新property同样是响应式的，且触发视图更新。它必须用于向响应式对象中添加新的property，因为Vue无法探测普通的新增property（比如 this.myObject.newProperty = 'hi'）
...
**在下次 DOM 更新循环结束之后执行延迟回调**，在修改数据之后立即使用 nextTick 来获取更新后的 DOM。
 ...
 nextTick优先是微任务，优先是Promise.then 方法，这样可以避免多一次队列，进而少一次UI渲染，节省性能
...
- 虚拟Dom就是用Javascript对象来描述真实Dom，是对真实Dom的抽象
- 由于直接操作Dom代价太大且性能低，可以将Dom操作转化成对象操作。最终通过diff算法比对差异进行更新Dom
- 虚拟Dom不依赖真实平台环境，可以实现跨平台

### diff算法：

1. 调用 pitch方法，传入新旧虚拟DOM,开始同层对比
2. 调用 isSameNode 方法，对比新旧节点是否是同类型节点
3. 如果不同，新节点直接代替旧节点
4. 如果相同，调用patchNode进行对比节点
   - 如果旧节点和新节点都是文本节点，则新文本代替旧节点
   - 如果旧节点没有子节点，新节点有，则把子节点新增上去
   - 如果旧节点有子节点，新节点没有，则删除旧节点的子节点
   - 如果都有子节点，则调用updateChildren 方法进行新旧子节点的对比
   - 子节点对比为首尾对比法
